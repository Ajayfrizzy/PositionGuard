...
export async function GET(request: Request) {
  ...
  const rows = await getPrisma().user.findMany({ where: walletAuth && "session" in walletAuth ? { id: walletAuth.session.protectedAccountId } : undefined, include: { policies: { orderBy: { updatedAt: "desc" } }, snapshots: { orderBy: { capturedAt: "desc" }, take: 1 } }, orderBy: { createdAt: "asc" } });
