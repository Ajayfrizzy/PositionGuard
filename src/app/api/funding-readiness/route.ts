...
export async function GET(request: Request) {
  ...
  try {
    const query = z.object({ chainId: z.coerce.number().int().positive() }).parse(Object.fromEntries(new URL(request.url).searchParams));
    const account = await getPrisma().user.findUniqueOrThrow({ where: { id: auth.session.protectedAccountId } });
    ...
    return Response.json({ protectedAccountId: account.id, selectedCandidateId: selected.candidate?.id ?? null, readiness: selected.readiness, ux: mapFundingReadiness(selected.readiness), rejectedCandidates: selected.rejected }, { headers: { "Cache-Control": "no-store" } });
