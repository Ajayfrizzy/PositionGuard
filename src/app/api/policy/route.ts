...
export async function PUT(request: Request) {
  ...
  try {
    ...
    const raw = JSON.parse(text) as Record<string, unknown>; const { confirmAutonomous, ...policyInput } = raw; const policy = policySchema.parse(policyInput); if (policy.executionMode === "AUTONOMOUS" && confirmAutonomous !== true) return json({ error: { code: "AUTONOMOUS_CONFIRMATION_REQUIRED", message: "Explicit confirmation is required before autonomous execution can be enabled." } }, 409); const db = getPrisma(); const chain = getDefaultChain(); const configured = process.env.AAVE_WALLET_ADDRESS?.toLowerCase(); const user = configured ? await db.user.findUnique({ where: { walletAddress: configured } }) : await db.user.findFirst({ orderBy: { createdAt: "desc" } });
