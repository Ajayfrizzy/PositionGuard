...
export async function PUT(request: Request) {
  ...
  try {
    if (!request.headers.get("content-type")?.startsWith("application/json"))
      return json({ error: { code: "INVALID_CONTENT_TYPE" } }, 415);
    const text = await request.text();
    if (text.length > 8192) return json({ error: { code: "BODY_TOO_LARGE" } }, 413);
    const raw = JSON.parse(text) as Record<string, unknown>;
    const { confirmAutonomous, ...policyInput } = raw;
    const policy = policySchema.parse(policyInput);
    if (policy.executionMode === "AUTONOMOUS" && confirmAutonomous !== true)
      return json(
        {
          error: {
            code: "AUTONOMOUS_CONFIRMATION_REQUIRED",
            message:
              "Explicit confirmation is required before autonomous protection can be enabled.",
          },
        },
        409,
      );
    const db = getPrisma();
    const chain = getChain(auth.session.chainId);
    const user = await db.user.findUnique({ where: { id: auth.session.protectedAccountId } });
    if (!user)
      return json(
        {
          error: {
            code: "PROTECTED_WALLET_NOT_FOUND",
            message: "Capture the configured protected wallet before saving a policy.",
          },
        },
        404,
      );
      ...
      return json(
        {
          error: {
            code: "INVALID_POLICY",
            message:
              "Policy is invalid. Require target > warning > emergency > 1 and valid limits.",
          },
        },
        400,
      );
    return json(
      { error: { code: "POLICY_UPDATE_FAILED", message: "The policy could not be saved." } },
      503,
    );
  }
}
