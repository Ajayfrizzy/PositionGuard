...
export async function POST(request: Request) {
  const operator = authenticateOperator(request); const walletAuth = operator === "authorized" ? null : await requireRequestSession(request); if (walletAuth && "error" in walletAuth) return walletAuth.error;
  try {
    ...
    if (input.mode === "broadcast" && operator !== "authorized") return reply({ error: { code: "BROADCAST_REQUIRES_OPERATOR_AUTHORIZATION", message: "User sessions may request preparation and simulation only." } }, 403);
    const session = walletAuth && "session" in walletAuth ? walletAuth.session : null;
    const result = await executeProtection({ mode: input.mode, walletAddress: session?.walletAddress, chainId: session?.chainId, broadcastSecret: input.mode === "broadcast" ? request.headers.get("x-positionguard-broadcast-authorization") : null }); return reply(result);
  } catch (error) {
