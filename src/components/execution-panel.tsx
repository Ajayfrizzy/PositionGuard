...
export interface TimelineGroup { key: "analyze" | "validate" | "execute" | "verify"; title: string; description: string; steps: TimelineStep[]; state: TimelineState }
...
const timelineSections = [
  { key: "analyze", title: "Analyze", description: "Refresh position and calculate MEI", keys: ["refresh", "mei", "REFRESHING_POSITION", "CALCULATING_MEI"] },
  { key: "validate", title: "Validate", description: "Apply policy and safety checks", keys: ["policy", "sender", "funding", "simulation", "VALIDATING_POLICY", "VERIFYING_SENDER", "VERIFYING_BALANCE", "VERIFYING_ALLOWANCE", "SIMULATING"] },
  { key: "execute", title: "Execute", description: "Revalidate and submit through KeeperHub", keys: ["revalidation", "execution", "REVALIDATING", "READY_TO_EXECUTE", "BROADCASTING"] },
  { key: "verify", title: "Verify", description: "Confirm receipt, Aave event, and outcome", keys: ["receipt", "outcome", "VERIFYING_RECEIPT", "VERIFYING_AAVE_POSITION", "CONFIRMED"] },
] as const;
...
export function groupExecutionTimeline(steps: TimelineStep[]): TimelineGroup[] {
  return timelineSections.map(section => {
    ...
    const state: TimelineState = grouped.some(step => step.state === "failed") ? "failed" : grouped.some(step => step.state === "cancelled") ? "cancelled" : grouped.length > 0 && grouped.every(step => step.state === "complete") ? "complete" : grouped.some(step => step.state === "active") ? "active" : "pending";
    return { key: section.key, title: section.title, description: section.description, steps: grouped, state };
  });
}
export function ExecutionPanel({ execution, canRequest }: { execution: ExecutionView | null; canRequest: boolean }) {
  const [message, setMessage] = useState(""); const [messageIsError, setMessageIsError] = useState(false); const [busy, setBusy] = useState(false); const [token, setToken] = useState(""); const [live, setLive] = useState<ProtectionExecutionResult | null>(null);
  async function requestProtection() { setBusy(true); setMessage(""); setMessageIsError(false); setLive(null); try { const response = await fetch("/api/protection/execute", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ mode: "simulate" }) }); const body = await response.json() as ProtectionExecutionResult & { error?: { message?: string; code?: string } }; if (!response.ok) throw new Error(body.error?.message ?? body.error?.code ?? "Simulation failed safely."); setLive(body); setMessage(`Simulation passed. ${body.amount} ${body.asset} would reach projected HF ${body.projectedHealthFactor ?? "unbounded"}. Gas estimate: ${body.simulation.gasEstimate}.`); } catch (error) { setMessageIsError(true); setMessage(error instanceof Error ? error.message : "Protection request could not reach the server."); } finally { setBusy(false); } }
  ...
  return <div className="execution-panel"><div className="timeline-groups" aria-label="Protection execution stages">{groups.map((group, index) => <section className={`timeline-group ${group.state}`} key={group.key}><header><span className="timeline-number">{index + 1}</span><div><h3>{group.title}</h3><p>{group.description}</p></div><small>{group.state}</small></header><div className="timeline-details">{group.steps.map(step => <div className={`timeline-step ${step.state}`} key={step.key}><span>{step.state === "complete" ? <Icon name="check"/> : step.state === "active" ? <span className="spinner"/> : step.state === "failed" || step.state === "cancelled" ? "!" : ""}</span><p>{step.label}</p><small>{step.state}</small></div>)}</div></section>)}</div>
    <div className="execute-cta" aria-busy={busy}><p>Simulation reloads policy and Aave state, recomputes MEI, checks sender, funding, allowance, and KeeperHub without signing or broadcasting.</p><label htmlFor="execution-authorization">Execution authorization</label><input id="execution-authorization" type="password" autoComplete="off" value={token} onChange={event => setToken(event.target.value)} placeholder="Enter authorization code"/><button className="button primary" disabled={!canRequest || !token || busy} onClick={() => void requestProtection()}><Icon name="shield"/>{busy ? "Running safety checks…" : "Simulate protection"}</button>{busy && <p className="sr-only" role="status">Protection simulation is running.</p>}{message && <div className={`inline-message ${messageIsError ? "error" : "success"}`} role={messageIsError ? "alert" : "status"}>{message}</div>}</div>
  </div>;
}
