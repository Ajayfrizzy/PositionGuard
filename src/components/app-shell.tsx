...
const nav = [["/dashboard", "Overview", "grid"], ["/position", "Position", "position"], ["/protection", "Protection", "shield"], ["/scenario", "Scenario", "activity"], ["/activity", "Activity", "activity"], ["/notifications", "Notifications", "activity"], ["/settings", "Settings", "settings"]] as const;
export function AppShell({ children, protectedWallet, protectionAttention }: { children: React.ReactNode; protectedWallet: string | null; protectionAttention: boolean }) {
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <div className="sidebar-foot"><div className="avatar"><Icon name="shield"/></div><div><b>{protectedWallet ? "Protected Account" : "Wallet not connected"}</b><span title={protectedWallet ?? undefined}>{protectedWallet ? shortAddress(protectedWallet) : "Verify ownership"}</span></div>{!protectedWallet && <Link href="/onboarding">Connect</Link>}</div>
    </aside>
