...
export function generateCandidates(
  ...
  rawPolicy: ProtectionPolicy,
  rawContext: PolicyContext,
): CandidateAction[] {
  const p = positionSchema.parse(input),
    policy = validatePolicy(rawPolicy),
    context = policyContextSchema.parse(rawContext);
  const debt = units(p.totalDebtUsd),
    collateral = units(p.totalCollateralUsd),
    lt = units(p.liquidationThreshold),
    target = units(policy.targetHealthFactor);
  ...
  for (const type of ["REPAY_DEBT", "ADD_COLLATERAL"] satisfies InterventionType[]) {
    ...
    const balance = units(
      type === "REPAY_DEBT" ? p.availableDebtAssetBalanceUsd : p.availableCollateralAssetBalanceUsd,
    );
    ...
    const amounts = new Set([
      ...
      minimum,
      minimum + 1n,
      ceilDiv(minimum * 5n, 4n),
      balance,
      cap,
      units(policy.approvalRequiredAboveUsd),
    ]);
    for (const a of amounts) {
      ...
      result.push({
        id: `${type}:${amount}`,
        type,
        asset: type === "REPAY_DEBT" ? p.debtAsset : p.collateralAsset,
        amount,
        estimatedUsdValue: amount,
        expectedHealthFactor,
        reachesTarget,
        policyValidity: rejection === null,
        valid: rejection === null && reachesTarget,
        requiresApproval: rejection === null && requiresApproval(amount, policy),
        rejectionReason: rejection ?? (reachesTarget ? null : "BELOW_TARGET"),
        rank: 0,
      });
    }
  }
