...
const stressInputSchema = z.object({
  asset: z.string().min(1).max(64),
  percentageShock: z.number().finite().gt(-100).max(1000),
});
const SCALE = 1_000_000n;
const ceilDiv = (a: bigint, b: bigint) => (a + b - 1n) / b;

export function stressPosition(
  positionInput: PortfolioPosition,
  asset: string,
  ...
  const assets = position.assets.map((item) => {
    if (
      item.id.toLowerCase() !== parsed.asset.toLowerCase() &&
      item.symbol.toLowerCase() !== parsed.asset.toLowerCase()
    )
      return item;
    found = true;
    return { ...item, priceBase: ((BigInt(item.priceBase) * factor) / SCALE).toString() };
  });
  ...
  for (const item of assets) {
    const unit = 10n ** BigInt(item.decimals),
      price = BigInt(item.priceBase);
    const debtNumerator = BigInt(item.debtBalance) * price;
    totalDebt +=
      position.debtRounding === "up" ? ceilDiv(debtNumerator, unit) : debtNumerator / unit;
  ...
  const healthFactorWad =
    totalDebt === 0n ? null : ((weighted * 10n ** 18n) / (totalDebt * 10_000n)).toString();
  return {
    ...position,
    assets,
    totalDebtBase: totalDebt.toString(),
    weightedCollateralNumerator: weighted.toString(),
    healthFactorWad,
  };
}
...
export function analyzeStress(input: {
  ...
  return {
    ...
    currentHealthFactor:
      input.position.healthFactorWad === null
        ? null
        : formatUnits(BigInt(input.position.healthFactorWad), 18),
    projectedHealthFactor:
      stressedPosition.healthFactorWad === null
        ? null
        : formatUnits(BigInt(stressedPosition.healthFactorWad), 18),
    projectedRisk: result.riskLevel,
    targetHealthFactor: input.policy.targetHealthFactor,
    mei: result.selectedCandidate
      ? {
          action: result.selectedCandidate.type,
