...
export function checkLimits(
  ...
  if (
    (type === "REPAY_DEBT" && !policy.allowRepay) ||
  ...
  if (a + units(c.dailyAutonomousSpendUsd) > units(policy.maxDailyAutonomousAmountUsd))
    return "DAILY_LIMIT";
  if (
    c.lastAutonomousExecutionAtMs !== null &&
    c.nowMs - c.lastAutonomousExecutionAtMs < policy.interventionCooldownMinutes * 60000
  )
    return "COOLDOWN";
  return null;
}
