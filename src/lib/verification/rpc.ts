...
export async function verifyBaseRpc(
  ...
  try {
    ...
    const reserveRaw = await reader.read({
      address: chain.aavePoolAddress,
      abi: poolAbi,
    ...
    for (const assetSymbol of assetSymbolSchema.options) {
      ...
      if (!reserves.data.some((a) => isAddressEqual(a, token.address)))
      ...
      const [symbol, decimals, balance, config] = await Promise.all([
        ...
        reader.read({
          address: chain.aavePoolDataProviderAddress,
          abi: dataProviderAbi,
      ...
      if (
        ...
        config[0] !== BigInt(token.decimals)
      )
      ...
      tokens.push({
        ...
        address: token.address,
        decimals: token.decimals,
        erc20ReadVerified: true,
      });
    }
    if ((await reader.block(block.number)).hash !== block.hash)
      throw new AaveReadError("STATE_CHANGED");
    return {
      working: true,
