...
export class WebhookNotificationProvider implements NotificationProvider {
  constructor(
    private readonly url = process.env.POSITIONGUARD_WEBHOOK_URL,
    private readonly secret = process.env.POSITIONGUARD_WEBHOOK_SECRET,
  ) {}
  async deliver(event: NotificationEvent) {
    ...
    const response = await fetch(this.url, {
      ...
      headers: {
        ...
        ...(signature ? { "X-PositionGuard-Signature": `sha256=${signature}` } : {}),
      },
      body,
...
export async function notify(
  ...
  const provider =
    options.provider === undefined
      ? process.env.POSITIONGUARD_WEBHOOK_URL
        ? new WebhookNotificationProvider()
        : null
      : options.provider;
  const row = await store.notification.create({
    data: {
      userId: event.userId,
      type: event.type,
      title: event.title,
      ...
      webhookStatus: provider ? "PENDING" : "SKIPPED",
    },
  });
  ...
  try {
    ...
    await store.notification.update({
      ...
      data: {
        ...
        webhookAttempts: { increment: 1 },
        webhookLastError: error instanceof Error ? error.message.slice(0, 500) : "WEBHOOK_FAILED",
      },
    });
