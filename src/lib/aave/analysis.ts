...
export interface PositionAnalysis {
  ...
  fundingWallet: AavePosition["wallet"];
  contextAssumption: string;
  result: ReturnType<typeof evaluateProtection>;
}
export function analyzePosition(position: AavePosition, policyInput: unknown): PositionAnalysis {
  ...
  return {
    ...
    fundingWallet: position.wallet,
    contextAssumption:
      "Preview assumes zero autonomous spend and no prior execution. Wallet balances are not KeeperHub-authorized funding.",
    result: evaluateProtection(position.normalizedProtectionInput, policy, {
