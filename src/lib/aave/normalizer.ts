...
export function normalizeAavePosition(
  account: RawAccount,
  reserves: readonly RawReserve[],
  baseUnit: bigint,
  eMode: bigint,
): PortfolioPosition {
  ...
  if (reserves.some((r) => r.collateralEnabled && r.debtCeiling > 0n))
    blockers.push("ISOLATION_ESTIMATION_UNSUPPORTED");
  let weighted = 0n,
    collateral = 0n,
    debtDown = 0n,
    debtUp = 0n;
  for (const r of reserves) {
    ...
    if (r.collateralEnabled) {
      const value = tokenValueBase(r.suppliedBalance, r.price, r.decimals);
      collateral += value;
      weighted += value * r.liquidationThresholdBps;
    }
  ...
  if (debtDown !== account.totalDebtBase && debtUp !== account.totalDebtBase)
    blockers.push("DEBT_RECONCILIATION_FAILED");
  // Preserve the reserve-weighted numerator instead of multiplying by the rounded account-average LT.
  return {
    model: "portfolio-v2",
    healthFactorWad: account.totalDebtBase === 0n ? null : account.healthFactor.toString(),
    ...
    debtRounding: debtUp === account.totalDebtBase ? "up" : "down",
    analysisBlockers: blockers,
    assets: reserves.map((r) => ({
      id: r.asset,
      symbol: r.symbol,
      decimals: r.decimals,
      ...
      debtBalance: r.variableDebt.toString(),
      liquidationThresholdBps: Number(r.liquidationThresholdBps),
      canRepay: r.active && !r.paused,
      canSupply:
        r.collateralEnabled &&
        r.active &&
        !r.paused &&
        !r.frozen &&
        r.liquidationThresholdBps > 0n &&
        r.debtCeiling === 0n,
