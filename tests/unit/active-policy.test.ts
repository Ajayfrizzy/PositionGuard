...
describe("active protection policy loading", () => {
  it("loads and validates the enabled wallet-and-chain policy", async () => {
    ...
    const result = await loadActiveProtectionPolicy(
  ...
  it("loads policy before RPC and never substitutes preview defaults", async () => {
    ...
    const result = await prepareLiveProtectionAnalysis(
      { walletAddress: wallet, chainId: baseSepoliaConfig.chainId },
      { loadPolicy, getPosition },
    );
    expect(loadPolicy.mock.invocationCallOrder[0]).toBeLessThan(
      getPosition.mock.invocationCallOrder[0]!,
    );
